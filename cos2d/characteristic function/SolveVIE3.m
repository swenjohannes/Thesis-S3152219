function [y,Dalpha_y,t] = SolveVIE2(f, y0, alpha, T, N, M)
...
%
    ...
    dummy2 = y0 * t .^ (alpha - 1) ./ gamma(alpha);
    ...
    for k=0:N-1
        ...
        % Compute predictor:
        yp = dummy2(k + 2) + sum(b_j_kp1(js,k).*Dalpha_y(:,1:k+1),2)./gamma(alpha);
        
        % Compute solution:
        if k==0
            y(:,2) = dummy2(k + 2) + (a_0_kp1(k)*Dalpha_y(:,1) ...
                      + a_kp1_kp1*f(yp))./gamma(alpha);
        else
            y(:,k+2) = dummy2(k + 2)+ (a_0_kp1(k)*Dalpha_y(:,1) ...
                        + sum(Dalpha_y(:,2:k+1).*a_j_kp1(js(2:end),k),2)...
                        + a_kp1_kp1*f(yp))./gamma(alpha);
        end

        % Compute fractional derivative:
        Dalpha_y(:,k+2) = f(y(:,k+2)) - y0 * t(k + 2) ^-alpha / gamma(1 -alpha);
    end
